...
(setup (:package corfu)
  ...
  ;; :bind は :after-loaded なので corfu ロード後に評価される
  (:with-map corfu-map
    (:bind "C-n" corfu-next
           "C-p" corfu-previous
           "C-f" corfu-insert))
...
;; corfu-history は corfu 本体に同梱されているので :package は付けない
(setup corfu-history
  (with-eval-after-load 'corfu
    (corfu-history-mode)
...
** marginalia
- 補完候補の右側に注釈を出す (バッファなら 状態フラグ / サイズ / メジャーモード / ファイルパス)
- vertico が候補を並べ、consult が候補を作り、marginalia がその注釈を足す、という分担
#+BEGIN_SRC emacs-lisp
(setup (:package marginalia)
  ;; 区切りは 3 箇所に入る (候補↔フラグ / フラグ↔サイズ / サイズ↔モード↔パス)
  (setopt marginalia-separator " │ ")
  ;; ミニバッファ内で注釈の詳しさを循環させる (詳細→簡易→組み込み→無し)
...
(setup (:package ghostel)
  ...
  ;; :hook は現在の feature (ghostel) の hook = ghostel-mode-hook に足す
  (:hook ghostel-ime-mode)
...
(setup (:package org-superstar)
  ;; :hook-into は現在の func (org-superstar-mode) を org-mode-hook に足す
  (:hook-into org-mode))
#+END_SRC
