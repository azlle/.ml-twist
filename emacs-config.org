...
#+BEGIN_SRC emacs-lisp
;; setup.el のブートストラップ。
;; `:package' は setup.el 組み込みだと package.el を呼んでしまうので、
;; 何もしない no-op に差し替える。twist が init.el を静的に読んで
;; `(:package NAME)' からビルド対象を決めるので、宣言としてだけ意味を持つ。
(eval-when-compile
  (require 'setup)
  (require 'cl-lib)

  ;; https://www.emacswiki.org/emacs/SetupEl の snippet と同じもの
...
as OPTS to `setup-define'."
  ...
  (define-setup-macro :package (package)
    "Fake installation of PACKAGE.  twist がビルドするので何もしない。"
    :repeatable t
    :shorthand cadr))
#+END_SRC
